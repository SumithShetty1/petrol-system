...
export default function ProfileCard({
  ...
  const showOwner = !!profile.owner_name;

  const showManagerName =
    showManagerInfo && !!profile.manager_name;
  ...
  const rows = [
    {
      ...
      label: "Phone Number",
      value: profile.phone || profile.username || "—",
    },
  ...
  return (
    <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-lg">
      ...
      <div className="flex flex-col items-center mb-6 md:mb-8">
        ...
        {/* Status */}
        <div
          className={`mt-2 px-3 py-1 rounded-full text-xs md:text-sm font-medium flex items-center gap-1 ${
            isActive
      ...
      <div className="space-y-4 md:space-y-5">
        {rows.map((row, index) => (
          <InfoRow
            ...
            label={row.label}
            value={row.value}
            noBorder={index === rows.length - 1} // ✅ FIX
          />
        ))}
      </div>
  ...
  return (
    <div
      className={`flex items-center gap-3 md:gap-4 ${
        !noBorder ? "pb-3 md:pb-4 border-b border-gray-100" : ""
      }`}
    >
      <div
